  ...
  return (
    <header className={`bg-white principal-container flex justify-center w-full ${menuOpened ? 'z-20' : ''} ${isScrolling ? 'shadow-lg' : ''}`}>
      <div className={`justify-between w-full flex md:p-4 relative`}>
        <Link to="/" className={`leading-none text-2xl sm:block sm:text-center no-underline outline-none text-white hover:text-secondary-500 focus:text-secondary-500 w-60 p-2 md:p-0`}>
          <picture className="w-30">
            <source media="(max-width: 768px)" srcSet={logo}></source>
        ...
        <nav role="navigation" aria-label="main-navigation" className={`flex absolute flex-col w-full z-20 bg-white top-16 ${menuOpened ? 'overflow-auto h-auto' : 'overflow-hidden h-0'} md:h-auto md:relative md:top-0 md:flex-row md:w-auto md:z-0`}>
