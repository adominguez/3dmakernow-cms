...
const SearcherProductsList = ({
  ...
  const toggleSelectProduct = (evt) => {
    ...
    if (isProductSelected(value)) {
      setProductsSelected(
        productsSelected.filter((product) => product.ASIN !== itemSelected.ASIN)
      )
    } else {
    ...
    <>
      {error ? (
        <div className="p-4 text-red-500">
          Ha habido problemas al mostrar la información
        </div>
      ) : null}
            ...
            <div key={ASIN} className="p-2">
              <div className="border rounded-sm">
                <div className="relative featured-thumbnail">
                  <button
                    className="absolute p-2 bg-red-100 rounded-3xl right-2 top-2"
                    onClick={() =>
                      setProductsSelected(
      ...
      {loadingProducts ? (
        ...
        <div className="flex justify-end mx-4">
          <button
            className={`border border-blueGray-500 rounded-md mx-2 p-2 ${
              !products?.length ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          ...
          <button
            className={`border border-blueGray-500 rounded-md p-2 ${
              !isProductsEmpty() ? 'opacity-50 cursor-not-allowed' : ''
            }`}
      ...
      <ul className="flex flex-wrap px-4">
        {products?.length && !loadingProducts
          ? products.map((item) => {
              const { ASIN, ImageUrl, Title, Rating, TotalReviews, Price } =
                item
              return (
                <li key={ASIN} className="w-full p-2 xl:w-1/3 md:w-1/2">
                  <label
                    htmlFor="products"
                    className={`relative block w-full h-full border-2 rounded-md ${
                  ...
                  >
                    <input
                      ...
                      checked={isProductSelected(ASIN)}
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="px-2 overflow-auto">
                        <div className="text-gray-700 truncate-2-lines">
                          {Title}
                        </div>
                        <span className="pr-4 text-2xl text-center text-primary-500">
                          {Price}
                        </span>
                        <ProductsStars
                          amazonRate={Rating}
                          amazonRatings={TotalReviews}
                        />
                      </div>
                    </div>
                  </label>
