...
const CompareProducts = ({ compareProducts: {properties, products} = {}, informationButtonText }) => {
	...
	const renderTableAttributes = () => {
        ...
        <p className={`flex items-center justify-start h-12 px-4 -mt-px text-center text-primary-500 bg-gray-200`}>Producto</p>
				{
					properties && properties.length && properties.map((property, index) => (
						<p key={`productData-${index}`} className={`flex items-center justify-start h-12 px-4 -mt-px text-center text-textColor-500 ${isOdd(index) ? 'bg-gray-200' : 'bg-white'}`}>{capitalize(property)}</p>
					))
				}
			</div>
		)
