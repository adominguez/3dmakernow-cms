...
const SearchInput = ({ onfocusInput, onCancelSearch, opened }) => {
  ...
  const arrowPressKey = (event) => {
    ...
    if (keyCode === 38) {
      ...
      prevSibling && prevSibling.children.length
        ? prevSibling.children[0].focus()
        : setFocus()
    }
  }
  ...
  const renderListItem = (item, index) => {
    return (
      ...
      >
        <Link
          onKeyUp={arrowPressKey}
          to={item.url.slice(0, 1) === '/' ? item.url : `/${item.url}`}
          className="flex p-3 hover:bg-gray-200 focus:bg-gray-200"
        >
          {item.name}
        </Link>
      </li>
  ...
  const setLastPagesVisited = () => {
    ...
    const lastPages = local
      ? JSON.parse(local)
          .filter((item) => item.url !== actualUrlpath())
          .reverse()
          .slice(0, 5)
      : []
    return (
        ...
        {lastPages.length ? (
          <>
            <h3 className="text-xl text-primary-500">
              Ultimas páginas que has visto
            </h3>
            <ul className="my-2">
              {lastPages.map((item, index) => renderListItem(item, index))}
            </ul>
          </>
        ) : (
          ''
  ...
  return (
    <div
      className={`top-0 w-full py-5 px-1 md:p-4 ${
        opened
          ? 'md:absolute rounded-t-none fixed left-0 bg-white md:rounded-lg z-20'
          : 'absolute'
      }`}
    >
      <div
        className={`relative flex w-full h-10 rounded-3xl border-transparent border-2 hover:bg-gray-200 ${
          opened ? 'bg-gray-200 border-primary-500 border-2' : ''
        }`}
      >
      ...
      {opened ? (
        <div
          className={`flex flex-col w-full mt-2 overflow-y-auto bg-white rounded-b-md max-h-80`}
        >
          {keyword.length ? (
            <>
              <ul>
          ...
          {keyword.length && !filteredResults.length ? (
