...
const CustomSection = ({
  ...
  return (
    <section
      ...
      } ${showBorder ? 'border-blueGray-300 border-b' : null}`}
    >
      <div
        className={`flex flex-col w-full p-4 md:w-4/5 ${colors[backgroundColor].textColor}`}
      >
        {title ? (
        ...
        {sectionContent ? (
          <HTMLContent
            className="flex-1 my-2 text-lg font-light custom-section"
