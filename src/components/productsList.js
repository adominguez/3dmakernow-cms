...
const ProductsList = ({
  ...
  const renderContent = (item) => {
    ...
    return (
      <a
        ...
        className={`flex p-4 ${
          typeList === 'carousel' ? 'flex-wrap justify-around' : ''
        } ${
          typeList === 'list'
            ? 'border-0 hover:bg-primary-100'
            : 'border rounded-md'
        } hover:border-primary-800 hover:cursor-pointer focus:outline-none focus:bg-primary-100 focus:border-primary-800`}
      >
        <img
          src={ImageUrl}
          loading="lazy"
    ...
    return (
      <div className="flex items-center justify-end p-2 px-4 my-2 border-b-2">
        <div className="flex-1">
          <button
            className={`${
              typeList === 'grid' ? 'bg-primary-500 text-white' : null
            } border mx-1 rounded-md text-blueGray-600 p-2`}
          ...
          >
            <svg
              ...
              fill="none"
              viewBox="0 0 24 24"
              stroke={typeList === 'grid' ? '#fff' : '#49b2a1'}
            >
              <path
                strokeLinecap="round"
          ...
          <button
            className={`${
              typeList === 'list' ? ' bg-primary-500 text-white' : null
            } border mx-1 rounded-md text-blueGray-600 p-2`}
          ...
          >
            <svg
              ...
              fill="none"
              viewBox="0 0 24 24"
              stroke={typeList === 'list' ? '#fff' : '#49b2a1'}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          {type === 'carousel' ? (
            <button
              className={`${
                typeList === 'carousel' ? ' bg-primary-500 text-white' : null
              } border mx-1 rounded-md text-blueGray-600 p-2`}
              onClick={() => setTypeList('carousel')}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-6 h-6"
                fill="none"
                viewBox="0 0 24 24"
                stroke={typeList === 'carousel' ? '#fff' : '#49b2a1'}
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
        ...
        <div>
          <label htmlFor="order" className="block text-primary-500">
            Ordenar por
          </label>
          <select
            className="p-2 bg-transparent border rounded-md text-blueGray-500 border-blueGray-500"
            name="order"
            value={order}
  ...
  return (
    <>
      {productsList?.length ? showToolbar && renderSortProducts() : null}
      ...
      {productsList?.length && typeList === 'carousel' ? (
        <Slider {...settings}>
          {productsListOrdered().map((item, index) =>
