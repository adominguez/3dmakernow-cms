...
export const ProductsListControl = (props) => {
  ...
  useEffect(() => {
    ...
    const products =
      productsListValue &&
      JSON.parse(
        `[${productsListValue._tail.array.toString().replaceAll('Map ', '')}]`
      )
    if (products?.length) {
  ...
  const setChange = ({ parsedType, parsedProducts }) => {
    onChange({
      ...
      productsList:
        (parsedProducts?.length && parsedProducts) ||
        (productsSelected?.length && productsSelected) ||
    ...
    <>
      ...
      <div style={wrapper}>
        <div className="flex">
          <label
            htmlFor="type"
            className={`relative p-1 m-2 flex items-center border border-blueGray-300 rounded-md ${
              isTypeSelected('grid') ? 'bg-white' : 'bg-blueGray-200 opacity-50'
            }`}
          >
            <input
              type="radio"
              name="type"
              value="grid"
              className="absolute w-full h-full opacity-0 hover:cursor-pointer"
              checked={isTypeSelected('grid')}
              onChange={() => changeType('grid')}
            />
            <svg
          ...
          <label
            htmlFor="type"
            className={`relative p-1 m-2 flex items-center border border-blueGray-300 rounded-md ${
              isTypeSelected('carousel')
                ? 'bg-white'
                : 'bg-blueGray-200 opacity-50'
            }`}
          >
            <input
              type="radio"
              ...
              value="carousel"
              className="absolute w-full h-full opacity-0 hover:cursor-pointer"
              checked={isTypeSelected('carousel')}
              onChange={() => changeType('carousel')}
            />
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="w-6 h-6"
          ...
          <label
            htmlFor="type"
            className={`relative p-1 m-2 flex items-center border border-blueGray-300 rounded-md ${
              isTypeSelected('list') ? 'bg-white' : 'bg-blueGray-200 opacity-50'
            }`}
          >
            <input
              type="radio"
              ...
              value="list"
              className="absolute w-full h-full opacity-0 hover:cursor-pointer"
              checked={isTypeSelected('list')}
              onChange={() => changeType('list')}
            />
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="w-6 h-6"
        ...
        <div>
          <label>
            ...
            />
            <button
              className={`p-2 bg-blueGray-600 text-white rounded-md my-2 ${
                !keyword ? 'opacity-50 cursor-not-allowed' : ''
              }`}
              disabled={!keyword}
              onClick={() => getAmazonInformation()}
            >
              Ver productos
            </button>
          </label>
        </div>
        <SearcherProductsList
          error={error}
          loadingProducts={loading}
          products={productsList}
          productsSelected={productsSelected}
          setProductsSelected={(products) => addProducts(products)}
        />
      </div>
    </>
  )
