...
export const CompareProductsControl = (props) => {
  ...
  const { compareProducts } =
    props.entry.getIn(['data']).toJS().comparation || {}
  ...
  useEffect(() => {
    if (compareProducts) {
      if (
        ...
        compareProducts?.properties?.length !== properties?.length
      ) {
        setProperties(compareProducts.properties)
      }
      if (
        compareProducts.products &&
        compareProducts?.products?.length !== products?.length
      ) {
        const newProducts = compareProducts.products.map(
          ({ propertiesValues, ...rest }) => {
            const comparation = {}
      ...
      products:
        ...
        convertedProducts.map((product) => {
          ...
          return {
            ...
            image,
            link,
            propertiesValues: Object.values(rest),
          }
        }),
    ...
    <div style={wrapper}>
      <div className="flex items-end my-2">
        <label htmlFor="prop">
          ...
          <InputWidget
            ...
            name="key"
            value={newProperty}
            onChange={(value) => setNewProperty(value)}
            classNameWrapper="css-83wr9v"
          />
        </label>
        <button
          className={`p-2 bg-blueGray-600 text-white rounded-md my-2 mx-4`}
          onClick={() => addNewProperty()}
        >
          Añadir propiedad a comparar
        </button>
      </div>
      {properties && properties.length ? (
        <ul>
          {properties.map((item, index) => (
            <li key={index} className={`flex justify-between border-b`}>
              <div className="flex items-center px-2 text-blueGray-500">
                {item}
              </div>
              <button
                className={`p-2 bg-white text-red-500 rounded-md my-3 mx-4`}
        ...
        <div className="flex text-blue-400">
          ...
          Todavía no hay ninguna propiedad, comienza creando una nueva propiedad
        </div>
      )}
      <div className="flex justify-end">
        <button
          ...
          } p-2 border-blueGray-600 border text-blueGray-600 rounded-md my-2 mx-4`}
  ...
  const isProductValidated = () => {
    ...
    return (
      ...
      product.name &&
      product.image &&
      !validateProperties.includes(false)
    )
  }
    ...
    <div style={wrapper}>
      {product ? (
        <>
            ...
            properties.map((item, index) => (
              <label key={index} htmlFor={index}>
                ...
                <InputWidget
                  ...
                  name={item}
                  value={product[item]}
                  onChange={(value) => setProductKey(item, value)}
                  classNameWrapper="css-83wr9v"
                />
              </label>
            ))}
          <div className="flex justify-end">
            <button
              className={`p-2 border border-blueGray-600 text-blueGray-600 rounded-md my-2 mx-4`}
        ...
        <>
          ...
          {products && products.length ? (
            <ul>
              {products.map((item, index) => (
                <li key={index} className={`flex justify-between border-b`}>
                  ...
                  <div>
                    <button
                      className={`p-2 bg-white text-blue-400 rounded-md my-3 mx-4`}
            ...
            <div className="flex text-blue-400">
              ...
              Todavía no hay ningún producto, crea un nuevo producto para
              comparar
            </div>
          )}
        </>
      ...
      <div className="flex">
        <button
          className={`p-2 border-blueGray-600 border text-blueGray-600 rounded-md my-2 mx-4`}
  ...
  return (
    <>
      ...
      {view === 'properties' ? renderProperties() : renderProducts()}
    </>
  )
}
